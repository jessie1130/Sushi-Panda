...
if (!empty($_GET['search']) && isset($_GET['ser_btn'])) {
    ...
    if ($result1 = mysqli_query($link, "SELECT * FROM checkout where account like '%$find%' ")) {
        $num = mysqli_num_rows($result1); //查詢結果筆數
        $pages = ceil($num / 1);
        if (!empty($_GET['page']))
            mysqli_data_seek($result1, ($_GET['page'] - 1) * 1);
        for ($j = 0; $j < 1; $j++) {
            ...
            if (!empty($row)) {
                ...
                <thead>
                    <form action='delete_checkout.php' method='POST'>
                ...
                for ($i = 0; $i < count($product); $i++) {
                    $result2 = mysqli_query($link, "SELECT * FROM product where p_name ='$product[$i]'");
    ...
    if ($result = mysqli_query($link, "SELECT * FROM checkout ")) {
        $data = "";
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12 col-lg-8">
                        ...
                        <h2>訂單查詢 <a href="#" class="search-nav" style="font-size: 20px; color:gray;"><img src="img/core-img/search.png" width="20" height="20" alt="">搜尋會員帳號</a></h2>
                    </div>
